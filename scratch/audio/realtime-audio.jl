...
function micmeter(; x_offset=0, y_offset=0, xtick=100, ytick=1000, xmax=10_000)
    ...
    x = x_offset
    send_plotter_cmds(plotter_port, ["PA$x,$(y_offset)", "PD"]; safety_up, outfile)
    while true
        ...
        @debug x y 
        send_plotter_cmds(plotter_port, ["PA $x,$y"]; safety_up, outfile)

        x += xtick
        if x >= xmax 
            x = x_offset
            y_offset += ytick
            send_plotter_cmds(plotter_port, ["PU", "PA$x,$(y_offset)", "PD"]; safety_up, outfile)
        end
    end
end
...
send_plotter_cmds(plotter_port, ["IN", "SP3", "PA0,0", "PU"]; safety_up, outfile)
micmeter(; x_offset=0, y_offset=500)

send_plotter_cmds(plotter_port, ["PU", "PA 0,0", "SP0"]; safety_up, outfile)
